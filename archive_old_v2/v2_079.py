...
# M2: half-half correlation
def half_corr(x):
    half = len(x)//2
    return float(np.corrcoef(x[:half], x[half:])[0,1])
...
# M4: amplitude-binned transition asymmetry
def amp_transition_asymmetry(x, k=5):
    bins = np.percentile(x, np.linspace(0, 100, k+1)[1:-1])
    states = np.digitize(x, bins)
    n = k + 1
    C = np.zeros((n, n))
...
for domain, sig in signals.items():
    for variant, tf in TRANSFORMS.items():
        ...
        rows.append({"domain": domain, "variant": variant,
                     ...
                     "m3": signed_compress(x), "m4": amp_transition_asymmetry(x)})
...
with open("v2_079_results.json", "w") as f:
    json.dump(summary, f, indent=2)
print("\nSaved: v2_079_results.json")
