...
# M2: nearest-neighbor cross-prediction asymmetry
def nn_prediction_asymmetry(x, m=5, tau=2):
    ...
    for t in range(n_emb):
        ...
        nn = np.argmin(dists)
        if nn + m*tau < N:
            pred_fwd.append(x[nn + m*tau])
        if nn - 1 >= 0 and t - 1 >= 0:
            pred_bwd.append(x[nn - 1])
    if len(pred_fwd) == 0 or len(pred_bwd) == 0:
        return 0.0
    fwd_err = np.mean((x[m*tau:m*tau+len(pred_fwd)] - np.array(pred_fwd))**2)
    bwd_err = np.mean((x[:len(pred_bwd)] - np.array(pred_bwd))**2)
...
with open("v2_076_results.json", "w") as f:
    json.dump(summary, f, indent=2)
print("\nSaved: v2_076_results.json")
