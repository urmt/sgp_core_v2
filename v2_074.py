...
# M1: signed ordinal flow — net destination minus source index
...
# M2: signed third-order time irreversibility (no abs)
...
# M3: signed compressibility asymmetry (no abs)
...
# M4: signed phase anisotropy (no abs)
...
for domain, sig in signals.items():
    for variant, tf in TRANSFORMS.items():
        ...
        m2 = signed_time_irreversibility(x)
...
with open("v2_074_results.json", "w") as f:
    json.dump(summary, f, indent=2)
print("\nSaved: v2_074_results.json")
