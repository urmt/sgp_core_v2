...
Compute C on MHD fields with multiple transforms and dimensional reductions.
"""
...
def main():
    """Run MHD C Computation with Transform + Dimension Audit."""
    
    # Create output directory
    output_dir = Path('/home/student/sgp_core_v2/audits/rd_well7b')
    output_dir.mkdir(exist_ok=True)
    
    print("=" * 60)
    print("RD-WELL.7B — MHD C Computation with Transform + Dimension Audit")
    print("=" * 60)
    ...
    for file_idx, file_path_rel in enumerate(files[:5]):  # Use first 5 files
        ...
        if field_3d is None:
            print(f"  Failed to load 3D field")
            continue
        ...
        for slice_name, slice_data in slices.items():
            ...
            # (C needs a sequence, not a single frame)
            sequence = [slice_normalized + np.random.normal(0, 0.01, slice_normalized.shape) for _ in range(10)]
            ...
            print(f"  {slice_name}: C_original={C_original:.4f}, C_rank={C_rank:.4f}, C_zscore={C_zscore:.4f}")
        
        # Compute C for 3D volume (using middle slice as proxy)
        volume_normalized = normalize_to_01(field_3d[mid_z, :, :])
        sequence_volume = [volume_normalized + np.random.normal(0, 0.01, volume_normalized.shape) for _ in range(10)]
        ...
        for slice_name in ['xy', 'xz', 'yz']:
            ...
            for result in all_results:
                if result['file_index'] == file_idx and result['slice'] == slice_name:
                    result['C_volume'] = float(C_volume)
                    result['ΔC_dimension'] = abs(result['C_original'] - C_volume)
                    break
        
        print(f"  volume: C_volume={C_volume:.4f}")
    
    # Save results
    with open(output_dir / 'mhd_c_results.json', 'w') as f:
        json.dump(all_results, f, indent=2)
    
    print(f"\nResults saved to mhd_c_results.json")
    ...
    if all_results:
        ...
        with open(output_dir / 'mhd_transform_results.json', 'w') as f:
            json.dump(summary, f, indent=2)
        
        print(f"\nSummary saved to mhd_transform_results.json")
    ...
    # Record hidden variable candidates
    hidden_variables = {
        'spectral_transport': {
    ...
    print(f"\nMHD C Computation Complete")
    print(f"  Files processed: 5")
    print(f"  Slices per file: 3")
    ...
    print(f"\nOutput Files:")
    print(f"  {output_dir / 'mhd_c_results.json'}")
    print(f"  {output_dir / 'mhd_transform_results.json'}")
    print(f"  {output_dir / 'hidden_variables.json'}")
    
    print("\n" + "=" * 60)
    print("RD-WELL.7B COMPLETE")
    print("=" * 60)
